...
contract LaunchpadFacet is ReentrancyGuard, Power {
     ...
     */
    function createCampaign(string memory _name, string memory _symbol, uint256 _targetFunding, uint32 _reserveRatio)
        external
    {
    ...
    function buyIn(uint256 _campaignId, uint256 _usdcAmount) external nonReentrant {
        ...
        uint256 tokensToMint =
            _calculatePurchaseReturn(TOKENS_FOR_SALE, campaign.amountRaised, campaign.reserveRatio, _usdcAmount);
        ...
        if (campaign.tokensSold + tokensToMint > TOKENS_FOR_SALE) {
            ...
            uint256 currentUsdc =
                (campaign.targetAmount * 10 ** 6 * (campaign.tokensSold ** 2)) / (TOKENS_FOR_SALE ** 2);
    ...
    function getTokensToMint(uint256 _supply, uint256 _reserveBalance, uint32 _reserveRatio, uint256 _depositAmount)
        external
        view
        returns (uint256)
    {
        return _calculatePurchaseReturn(_supply, _reserveBalance, _reserveRatio, _depositAmount);
    }
    ...
    function _addLiquidity(uint256 campaignId, uint256 usdcAmount) internal {
        ...
        require(
            IERC20(address(campaign.token)).approve(address(diamond.uniswapRouter), LIQUIDITY_ALLOCATION),
            "approve failed."
        );
